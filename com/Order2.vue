<template>
	<div id="orederContent">
		<ul class="MyCartPro">
			<li class="prolist" v-for="item in cartPros">
				<div class="cartRight">
					<p>{{item.name}}</p>
					<p><span class="cartproPric">￥{{item.price}}</span></p>
					<p class="ProNum"><span v-model="num">{{item.num}}</span></p>
				</div>
			</li>
	    </ul>
	    <div class="cartfooter">
	    	...
	    	<div class="subPrice">
	    		合计：<span v-model="num">{{num*cartPros.price/100}}</span>
	    	</div>
